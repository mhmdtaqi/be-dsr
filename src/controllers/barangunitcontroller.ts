...
export const barangUnitController = {
      ...
      if (err instanceof Prisma.PrismaClientKnownRequestError) {
        ...
        if (err.code === "P2003") {
          res.status(400).json({
            success: false,
            message:
              "Foreign key constraint gagal. Pastikan kode barang, lokasi, dan NIK user valid",
          });
          return;
        }
      }
      ...
      if (err instanceof Prisma.PrismaClientKnownRequestError) {
        if (err.code === "P2003") {
          res.status(409).json({
            success: false,
            message:
              "Tidak dapat menghapus barang unit karena masih memiliki data terkait (monitoring atau peminjaman)",
          });
          return;
        }
        if (err.code === "P2025") {
  ...
  findAvailableForPeminjaman: async (
    ...
    try {
      ...
      console.error("Find available for peminjaman error:", err);
      res.status(500).json({
        success: false,
        message: "Terjadi kesalahan server",
      });
