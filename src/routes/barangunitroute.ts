...
router.post(
  ...
  [
    ...
    body("status")
      ...
      .withMessage(
        `Status harus salah satu dari: ${Object.values(StatusB).join(", ")}`
      ),
  ],
  validate,
...
router.get(
  ...
  [
    ...
    query("lokasi").optional().trim(),
    query("kodeBarang").optional().trim(),
  ],
  validate,
  barangUnitController.findAll
);

// ✅ GET: daftar barang yang boleh dipinjam (civitas & admin)
router.get(
  "/available-for-peminjaman",
  authMiddleware,
  authorize([
    Role.civitas_faste,
    Role.staff,
  ...
  barangUnitController.findAvailableForPeminjaman
);

// GET One: civitas & admin boleh lihat detail satu barang
router.get(
  "/:nup",
  authMiddleware,
  authorize([
