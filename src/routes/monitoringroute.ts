...
router.post(
  ...
  upload.single("foto"),
  [
    body("nupBarang").notEmpty().withMessage("NUP barang wajib diisi").trim(),
    body("waktu")
      .notEmpty()
    ...
    body("kondisiBarang")
      ...
      .withMessage(
        `Kondisi barang harus salah satu dari: ${Object.values(
          KondisiBarangM
        ).join(", ")}`
      ),
    body("lokasiBarang").optional().trim(),
    body("lokasiTambahan")
      .optional()
      .trim()
...
router.get(
  ...
  [
    ...
    query("lokasiBarang").optional().trim(),
    query("startDate")
      .optional()
      .isISO8601()
...
router.get(
  ...
  authMiddleware,
  authorize([Role.staff, Role.staff_prodi, Role.kepala_bagian_akademik]),
  [param("nup").notEmpty().withMessage("NUP barang wajib diisi").trim()],
  validate,
...
router.get(
  ...
  authMiddleware,
  authorize([Role.staff, Role.staff_prodi, Role.kepala_bagian_akademik]),
  [param("id").notEmpty().withMessage("ID monitoring wajib diisi").trim()],
  validate,
  monitoringController.findOne
);

// PUT: Staff, Staff Prodi, dan Kepala Bagian Akademik boleh mengubah log
router.put(
  "/:id",
  authMiddleware,
  authorize([Role.staff, Role.staff_prodi, Role.kepala_bagian_akademik]),
